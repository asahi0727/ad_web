...
- **AffiliateBox**: `<AffiliateBox provider="rakuten-travel" query="羽田空港 ホテル" label="..." />`。`site.config.ts` の ID を使ってリンクボックスを描画。ID が未設定なら通常リンクとして描画し、サイトは壊れない。`rel="sponsored noopener"` を必ず付ける。
- **AdSlot**: AdSense のクライアント ID が設定されているときのみ広告枠を描画。未設定なら何も出力しない。
- **Eyecatch**: カテゴリとタイトルからカテゴリ別配色の SVG を生成。OGP 画像としても使用。
- **TOC**: h2/h3 から目次を生成。
- **PostCard**: 一覧用カード。
...
- AffiliateBox は 1 記事 1〜3 個。本文の流れに沿った位置に置く
- 内部リンクは最低 2 本
